...
import { Link, useNavigate, useParams } from 'react-router-dom';
import toast from 'react-hot-toast';
import { ChevronRight, Pencil, Trash2, Power, Zap, Shield, Key, Lock } from 'lucide-react';
import PageHeader from '../../components/layout/PageHeader';
import Card, { CardHeader } from '../../components/common/Card';
import Button from '../../components/common/Button';
import Loader from '../../components/common/Loader';
...
export default function EndpointDetails() {
  ...
  // V1.5 — Build authentication display info
  const authInfo = endpoint.hasAuthentication ? {
    type: endpoint.authType,
    label: AUTH_TYPE_LABELS[endpoint.authType] || endpoint.authType,
          ...
          <dl className="space-y-3 text-sm">
            ...
            <div className="flex justify-between border-t border-border pt-2">
              ...
              <dd className="text-text-primary">
                {authInfo ? (
                  <AuthTypeBadge authType={authInfo.type} />
                ) : (
                  <span className="text-text-muted text-xs">None</span>
                )}
              </dd>
            </div>
            {authInfo?.type === 'LOGIN_FLOW' && authInfo.loginConfig && (
              <div className="border-t border-border pt-2">
                <p className="text-xs font-semibold text-text-secondary mb-1">Login Flow Configuration</p>
                <dl className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    <dt className="text-text-muted">Login URL</dt>
                    <dd className="text-text-primary truncate max-w-[60%]">{authInfo.loginConfig.loginUrl || '—'}</dd>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <dd className="text-text-primary font-mono-code">{authInfo.loginConfig.tokenPath || 'data.accessToken'}</dd>
                  </div>
                  <div className="flex justify-between">
